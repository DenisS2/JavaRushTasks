...
/*public class Solution {
    ...
    public static void main(String[] args)  {
        ...
        String fileName = sc.nextLine(); //"d://3.txt";//
        sc.close();
        String s = "";
        try {
        ...
        for (int i = 0; i < sa.length; i++) {
            if (!sa[i].equals(""))
            sba.add(new StringBuilder(sa[i]));
        }
        int length=sba.size();
            try {
                for (int i = 0; i < length; i++) {
                    for (int j = 0; j < length; j++) {
                        if (sba.get(i).equals(sba.get(j).reverse())) {
                            Pair pair = new Pair();
                            pair.first = sba.get(j).toString();
                            pair.second = sba.get(i).reverse().toString();
                            result.add(pair);
                            sba.remove(i);
                            sba.remove(j);
                            length = length - 2;
